...
@section('title', 'Edit Receipt ' . $goldReceipt->receipt_no)
...
@section('content')
<form action="{{ route('gold-receipts.update', $goldReceipt) }}" method="POST" id="receipt-form">
    @csrf
    @method('PUT')
        ...
        <div class="form-column">
            <div class="page-header" style="margin-bottom: 28px;">
                <div class="page-title-group">
                    <h1>Edit Receipt {{ $goldReceipt->receipt_no }}</h1>
                    <p class="font-urdu" style="margin-top:4px;">رسید میں ترمیم</p>
                </div>
            </div>
                ...
                <div class="input-grid">
                    <div class="form-group full-width">
                        ...
                        <select name="receipt_type" id="receipt_type" required>
                            <option value="customer" {{ $goldReceipt->receipt_type === 'customer' ? 'selected' : '' }}>Customer Receipt (گاہک)</option>
                            <option value="dukandar" {{ $goldReceipt->receipt_type === 'dukandar' ? 'selected' : '' }}>Dukandar Receipt (دوکاندار)</option>
                            <option value="karigar" {{ $goldReceipt->receipt_type === 'karigar' ? 'selected' : '' }}>Karigar Receipt (کاریگر)</option>
                        </select>
                    </div>
                    <div class="form-group full-width">
                        ...
                        <select name="customer_id" id="customer_id" class="filter-control" required>
                            @foreach($customers as $customer)
                                <option value="{{ $customer->id }}" {{ $goldReceipt->customer_id == $customer->id ? 'selected' : '' }}>{{ $customer->name }} ({{ ucfirst($customer->party_type) }})</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="receipt_date">Date <span class="font-urdu">تاریخ</span></label>
                        <input type="date" name="receipt_date" id="receipt_date" class="filter-control" value="{{ $goldReceipt->receipt_date->format('Y-m-d') }}" required>
                    </div>
                    <div class="form-group">
                        <label>Receipt No <span class="font-urdu">رسید نمبر</span></label>
                        <input type="text" class="filter-control" value="{{ $goldReceipt->receipt_no }}" readonly style="background:var(--bg-app); color:var(--gold-bright); font-weight:700;">
                    </div>
                </div>
            </div>
                    ...
                    @foreach($goldReceipt->items as $item)
                        <div class="receive-row" data-index="{{ $loop->index }}">
                            <div class="form-group">
                                ...
                                <input type="text" name="items[{{ $loop->index }}][description]" class="filter-control" value="{{ $item->description }}" placeholder="e.g. Old Chain">
                            </div>
                            <div class="form-group">
                                <label style="font-size:0.7rem; text-transform:none; letter-spacing:0;">Gross Wt <span class="font-urdu">کچا وزن</span></label>
                                <div class="calc-input-wrapper">
                                    <input type="number" step="0.001" name="items[{{ $loop->index }}][gross_weight]" class="filter-control receive-gross" placeholder="0.000" value="{{ $item->gross_weight }}">
                            ...
                            <div class="form-group">
                                ...
                                <div class="calc-input-wrapper">
                                    <input type="number" step="0.01" name="items[{{ $loop->index }}][ratti_impurity]" class="filter-control receive-ratti" placeholder="0" value="{{ $item->ratti_impurity }}">
                            ...
                            <div class="form-group">
                                ...
                                <div class="calc-input-wrapper">
                                    <input type="number" step="0.001" name="items[{{ $loop->index }}][khalis_weight]" class="filter-control receive-khalis" placeholder="0.000" value="{{ $item->khalis_weight }}" readonly style="background: var(--bg-app); color: var(--gold-bright); font-weight: 700;">
                                    <span class="unit-label">g</span>
                                </div>
                            </div>
            ...
            <div class="form-section">
                ...
                <div class="form-group full-width" style="margin-bottom:0;">
                    ...
                    <textarea name="remarks" id="remarks" class="filter-control" rows="2" placeholder="Any additional notes...">{{ $goldReceipt->remarks }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="sidebar-column">
            <div class="live-panel">
                ...
                <div class="live-row highlight-gold" id="total-gross-row">
                    ...
                    <span class="live-value" id="live-total-gross">{{ number_format($goldReceipt->total_gross_weight, 3) }}g</span>
                </div>
                <div class="live-row" style="border-top: 1px solid rgba(255,255,255,0.05); margin-top:8px; padding-top:12px;">
                    ...
                    <span class="live-value" id="live-total-deduction">{{ number_format($goldReceipt->total_gross_weight - $goldReceipt->total_khalis_weight, 3) }}g</span>
                </div>
                <div class="live-row highlight-gold" id="total-khalis-row">
                    <span class="live-label"><i class="bi bi-star-fill"></i> Total Khalis:</span>
                    <span class="live-value" id="live-total-khalis">{{ number_format($goldReceipt->total_khalis_weight, 3) }}g</span>
                </div>

                <div class="total-box">
                    <div class="total-label">PURE GOLD RECEIVED</div>
                    <div class="total-value" id="val-total-khalis">{{ number_format($goldReceipt->total_khalis_weight, 3) }}g</div>
                </div>
                ...
                <div style="margin-top: 28px; display: grid; gap: 14px;">
                    ...
                    <a href="{{ route('gold-receipts.show', $goldReceipt) }}" class="button-outline" style="justify-content: center; padding: 12px; text-align:center;">
...
<script>
    ...
    let receiveRowIndex = {{ $goldReceipt->items->count() }};
